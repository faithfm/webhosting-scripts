...
if [[ -f $WH_SITE_NGINX_FILE ]]; then
    ...
    if [[ -z "$WH_PHP_CMD" ]]; then
        # Look for include directive pointing to forge-conf/*/site.conf
        FORGE_CONF_INCLUDE=$(grep '^\s*include\s.*forge-conf/.*/site\.conf' "$WH_SITE_NGINX_FILE" | awk '{print $2}' | tr -d ';')
        if [[ -n "$FORGE_CONF_INCLUDE" ]]; then
...
while [[ "$SEARCH_DIR" != "/" && "$SEARCH_DIR" != "." ]]; do
    if [[ -d "$SEARCH_DIR/.git" ]]; then
        ...
        # check for vite.config as a js, mjs, or ts file
        if [[ -f "$WH_PROJECT_DIR/vite.config.mjs" || -f "$WH_PROJECT_DIR/vite.config.js" || -f "$WH_PROJECT_DIR/vite.config.ts" ]]; then
