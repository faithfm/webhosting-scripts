...
- **`wp` runs under the site's own PHP** (`WH_PHP_CMD`, from nginx) — the same mechanism as `wh wp`. On multi-PHP servers plain `wp` may run under an older CLI PHP that current plugins refuse. Both scripts resolve the wp-cli **phar** via `WH_WP_PHAR`, never `command -v wp`: where `/usr/local/bin/wp` is a shell wrapper, handing it to PHP would echo the wrapper's own source instead of running wp-cli, and every read here would silently parse that text. Install or update it with `wh wp-install`.
